...
#     """Stima robusta del background su un singolo frame usando solo i pixel fuori ROI.
#        Usa median + MAD-clipping (±3σ robusti) e restituisce uno scalare."""
...
#     seima_cube = hdrdat[0].data  # shape: [N, Y, X]
#     texp_ms = head.get('T_EX_MS', None)
#     nframes = seima_cube.shape[0] if seima_cube.ndim == 3 else 1
#
#     # --- ROI della tua comparativa ---
#     y0, y1 = 400, 600
#     x0, x1 = 650, 1000
#
#     # --- maschera "fuori ROI" per la stima del background ---
...
#         corrected = seima_cube - bg
#         corrected = corrected[None, ...]  # uniforma a 3D
#
#     # --- estrazione ROI: short vs long ---
#     short_img = corrected[0, y0:y1, x0:x1]
#     long_img  = corrected.mean(axis=0)[y0:y1, x0:x1]
#
#     # --- normalizzazione lineare condivisa (stessa scala per entrambi) ---
...
#
#     # unico colorbar condiviso (spanning entrambe le assi)
#     cbar = fig.colorbar(im1, ax=axes, location='right', fraction=0.045, pad=0.02)
#     cbar.set_label("ADU")  # scala lineare
#
#     for ax in axes:
...
#         fig.suptitle(f"Seeing-limited PSF vs zero-order — ROI [{y0}:{y1}, {x0}:{x1}]  |  t_exp={texp_ms} ms",
#                      fontsize=12, y=0.99)
#
#     plt.show()
