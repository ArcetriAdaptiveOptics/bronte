...
#         # fallback: se t50 manca, usa mid-point (t10+t90)/2 quando possibile
#         if (t50 is None) and (t10 is not None) and (t90 is not None):
#             t50 = 0.5*(t10 + t90)
#
#         if (t10 is None) or (t90 is None) or not (t90 > t10):
#             return None
#
...
#
#         return dict(channel=channel, kind=kind, idx=i0,
#                     t_center=float(self._t[i0]),
#                     t10=float(t10), t50=float(t50) if t50 is not None else np.nan, t90=float(t90),
...
#                 j = self.refine_index_abs(t_pred, channel, search_ms=search_ms)
...
#         diffs_to_PD1 = [out_PD1["rise"][i]["width_10_90_ms"] - out_PD2["fall"][j]["width_10_90_ms"] for i,j in p_r_f]
#
...
#         diffs_to_PD2 = [out_PD1["fall"][i]["width_10_90_ms"] - out_PD2["rise"][j]["width_10_90_ms"] for i,j in p_f_r]
#
#         def _stats(v):
...
#         )
#
#     # ---------- Latenza: metodo A (griglia su T noto) ----------
#     @staticmethod
#     def _wrap_residuals(times: np.ndarray, T: float, delta: float) -> np.ndarray:
#         x = times - delta
#         n = np.round(x / T)
#         r = x - n * T
#         return (r + T/2) % T - T/2
#
#     def estimate_command_latency(
#         self,
#         times: List[float],
#         dwell_time_s: float,
#         period_factor: int = 1,      # 1 = tutti gli eventi (T), 2 = solo rising o solo falling (2T)
#         grid_pts: int = 2001,
#         n_bootstrap: int = 0,
#         random_state: Optional[int] = None
#     ) -> Dict:
#         t = np.array(sorted(times), float)
#         N = len(t)
#         if N < 2 or dwell_time_s <= 0:
#             return dict(N=N, T_eff_s=np.nan, delta_s=np.nan, rms_ms=np.nan, std_ms=np.nan,
#                         residuals_ms=[], delta_std_ms=np.nan, delta_ci95_ms=(np.nan, np.nan))
#         T_eff = period_factor * dwell_time_s
#         deltas = np.linspace(0.0, T_eff, grid_pts, endpoint=False)
#         rms_arr = np.empty_like(deltas)
#         for i, d in enumerate(deltas):
#             r = self._wrap_residuals(t, T_eff, d)
#             rms_arr[i] = np.sqrt(np.mean(r*r))
#         j = int(np.argmin(rms_arr))
#         delta_star = float(deltas[j])
#         r = self._wrap_residuals(t, T_eff, delta_star)
#         rms = float(np.sqrt(np.mean(r*r)))
#         std = float(np.std(r, ddof=1)) if N > 1 else np.nan
#
#         # bootstrap opzionale
#         delta_samples = []
#         if n_bootstrap and N >= 3:
#             rng = np.random.default_rng(random_state)
#             for _ in range(n_bootstrap):
#                 tb = np.sort(rng.choice(t, size=N, replace=True))
#                 best = None; best_rms = None
#                 for d in deltas:
#                     rb = self._wrap_residuals(tb, T_eff, d)
#                     val = np.sqrt(np.mean(rb*rb))
#                     if (best is None) or (val < best_rms):
#                         best = d; best_rms = val
#                 delta_samples.append(best)
#         delta_std = float(np.std(delta_samples, ddof=1)) if len(delta_samples) > 1 else np.nan
#         if len(delta_samples) > 10:
#             lo, hi = (float(np.percentile(delta_samples, 2.5)), float(np.percentile(delta_samples, 97.5)))
#         else:
#             lo, hi = (np.nan, np.nan)
#
#         return dict(
#             N=N, T_eff_s=float(T_eff), delta_s=delta_star,
#             rms_ms=rms * 1000.0, std_ms=std * 1000.0,
#             residuals_ms=list(r * 1000.0),
#             delta_std_ms=(delta_std * 1000.0) if np.isfinite(delta_std) else np.nan,
#             delta_ci95_ms=(lo*1000.0 if np.isfinite(lo) else np.nan,
#                            hi*1000.0 if np.isfinite(hi) else np.nan),
#             method="grid_nominalT"
#         )
#
#     # ---------- Latenza: metodo B (regressione lineare, stima anche T_eff) ----------
#     def estimate_command_latency_linear(
#         self,
#         times: List[float],
#         period_factor_hint: int = 1  # 1 per (rise+fall), 2 per soli rise o soli fall
#     ) -> Dict:
#         """
#         Fit lineare: t_i ≈ delta + i*T_eff, con i = 0..N-1 dopo ordinamento.
#         Restituisce delta (normalizzato in [0, T_eff)), T_eff stimato, jitter (RMS e std) e N.
#         """
#         t = np.array(sorted(times), float)
#         N = len(t)
#         if N < 2:
#             return dict(N=N, T_eff_s=np.nan, delta_s=np.nan, rms_ms=np.nan, std_ms=np.nan, method="linear_fit")
#         i = np.arange(N, dtype=float)
#         i_mean = np.mean(i); t_mean = np.mean(t)
#         var_i = np.sum((i - i_mean)**2)
#         if var_i == 0:
#             return dict(N=N, T_eff_s=np.nan, delta_s=np.nan, rms_ms=np.nan, std_ms=np.nan, method="linear_fit")
#         T_eff = np.sum((i - i_mean)*(t - t_mean)) / var_i
#         delta = t_mean - T_eff * i_mean
#         # normalizza delta in [0, T_eff)
#         delta = (delta % T_eff) if T_eff > 0 else delta
#         r = t - (delta + T_eff*i)
#         rms = float(np.sqrt(np.mean(r*r)))
#         std = float(np.std(r, ddof=1)) if N > 1 else np.nan
#         # dwell effettivo stimato:
#         dwell_hat = T_eff / period_factor_hint
#         return dict(N=N, T_eff_s=float(T_eff), dwell_hat_s=float(dwell_hat),
#                     delta_s=float(delta), rms_ms=rms*1000.0, std_ms=std*1000.0, method="linear_fit")
#
#     # getter
#     @property
...
#             t50 = m['t50'] if np.isfinite(m['t50']) else float('nan')
#             print(f"{label[0]}{k:02d} @ t={m['t_center']:.6f}s | t50={t50:.6f}s | 10–90={m['width_10_90_ms']:.3f} ms ±{unc:.3f} ms"
...
#
#     # ---------- LATENZA (NO TRIGGER) SU PD1 — usando t50 ----------
#     times_PD1_all  = [m["t50"] for m in sorted(out1["rise"] + out1["fall"], key=lambda r: r["t_center"]) if np.isfinite(m["t50"])]
#     times_PD1_rise = [m["t50"] for m in out1["rise"] if np.isfinite(m["t50"])]
#     times_PD1_fall = [m["t50"] for m in out1["fall"] if np.isfinite(m["t50"])]
#
#     # Metodo A: griglia su T nominale
#     lat_all_grid  = rta.estimate_command_latency(times_PD1_all,  dwell_time_in_s, period_factor=1, grid_pts=4001, n_bootstrap=0)
#     lat_rise_grid = rta.estimate_command_latency(times_PD1_rise, dwell_time_in_s, period_factor=2, grid_pts=4001, n_bootstrap=0)
#     lat_fall_grid = rta.estimate_command_latency(times_PD1_fall, dwell_time_in_s, period_factor=2, grid_pts=4001, n_bootstrap=0)
#
#     # Metodo B: regressione lineare (stima anche T_eff)
#     lat_all_lin  = rta.estimate_command_latency_linear(times_PD1_all,  period_factor_hint=1)
#     lat_rise_lin = rta.estimate_command_latency_linear(times_PD1_rise, period_factor_hint=2)
#     lat_fall_lin = rta.estimate_command_latency_linear(times_PD1_fall, period_factor_hint=2)
#
#     print("\n--- Command latency on PD1 using t50 (no trigger) ---")
#     print(f"[A-grid, T]      all:  N={lat_all_grid['N']},  delta≈{lat_all_grid['delta_s']*1000:.2f} ms,  jitter_RMS≈{lat_all_grid['rms_ms']:.2f} ms, residual_std≈{lat_all_grid['std_ms']:.2f} ms")
#     print(f"[A-grid, 2T]     rise: N={lat_rise_grid['N']}, delta≈{lat_rise_grid['delta_s']*1000:.2f} ms,  jitter_RMS≈{lat_rise_grid['rms_ms']:.2f} ms, residual_std≈{lat_rise_grid['std_ms']:.2f} ms")
#     print(f"[A-grid, 2T]     fall: N={lat_fall_grid['N']}, delta≈{lat_fall_grid['delta_s']*1000:.2f} ms,  jitter_RMS≈{lat_fall_grid['rms_ms']:.2f} ms, residual_std≈{lat_fall_grid['std_ms']:.2f} ms")
#     print(f"[B-linear]       all:  N={lat_all_lin['N']},  delta≈{lat_all_lin['delta_s']*1000:.2f} ms,  T_eff≈{lat_all_lin['T_eff_s']*1000:.2f} ms  (~dwell {lat_all_lin.get('dwell_hat_s', np.nan)*1000:.2f} ms), jitter_RMS≈{lat_all_lin['rms_ms']:.2f} ms")
#     print(f"[B-linear]       rise: N={lat_rise_lin['N']}, delta≈{lat_rise_lin['delta_s']*1000:.2f} ms,  T_eff≈{lat_rise_lin['T_eff_s']*1000:.2f} ms  (~2*dwell {2*1000*dwell_time_in_s:.2f} ms?), jitter_RMS≈{lat_rise_lin['rms_ms']:.2f} ms")
#     print(f"[B-linear]       fall: N={lat_fall_lin['N']}, delta≈{lat_fall_lin['delta_s']*1000:.2f} ms,  T_eff≈{lat_fall_lin['T_eff_s']*1000:.2f} ms  (~2*dwell {2*1000*dwell_time_in_s:.2f} ms?), jitter_RMS≈{lat_fall_lin['rms_ms']:.2f} ms")
#
